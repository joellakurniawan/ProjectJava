 ...
 * @author JOELLA
 */
public class EditReturPembelianController {
    @FXML
    ...
    @FXML
    private TableView<addreturpenjualan_class> editreturpembelian_table;
    ...
    @FXML
    private JFXButton editreturpembelian_btn;

    @FXML
    private JFXButton editreturpembelian_cancel;
    ...
    @FXML
    private JFXButton deletedetailreturpembelian_btn;
    ...
    @FXML
    void initialize(returpembelian_class rp){
        ...
        try {
            ...
            this.notapembelian_id.setText(String.valueOf(rp.getId_notapembelian()));
            this.supplier_id.setText(rp.getNama_supplier());
            this.supplier_id2.setText(rp.getNama_supplier());
            this.returpembelian_id.setText(String.valueOf(rp.getId()));
            ...
            rs = statement.executeQuery("SELECT * FROM retur_pembelian WHERE id="+rp.getId());
            while (rs.next()){
                if(rs.getBoolean("potong_nota")){
                    potongnota_cb.setSelected(true);
                    potongtunai_cb.setSelected(false);
                    totalmasuk = rp.getTotal_harga();
                }
                else{
            ...
            rs = statement.executeQuery("SELECT DISTINCT * FROM detail_nota_pembelian dnp JOIN item i ON dnp.id_item=i.id WHERE dnp.id_notapembelian=" + notapembelian_id.getText().toString());
            while(rs.next()){
            ...
            editreturpembelian_table.setItems(datarp);
            rs.close();
    ...
    void initialize2(){
        ...
        Statement statement = null;
        try {
            editreturpembelian_table.getItems().clear();
            con = ConnectionFactory.getConnection();
            statement = con.createStatement();
            ...
            detailreturpembelian_id.setText(String.valueOf(masukk));
            rs.close();
        ...
        try{
                    if(item_cb.getValue().isEmpty()==false){
                        //if(Integer.parseInt(jumlah_txt.getText()) > Integer.parseInt(jumlah_textfield.getText())){
                            statement = con.createStatement();
                            int iditem=0;
                            ResultSet rs = statement.executeQuery("SELECT DISTINCT * FROM detail_nota_pembelian dnp JOIN item i ON dnp.id_item=i.id WHERE dnp.id_notapembelian="+notapembelian_id.getText().toString()+" AND i.nama='"+item_cb.getValue().toString()+"'");
                            while(rs.next()){
    ...
    @FXML
    void deletedetailreturpembelian_btnClick(ActionEvent event) {
        Connection con = null;
                Statement statement = null;
                con = ConnectionFactory.getConnection();
                try {
                    addreturpenjualan_class rp = (addreturpenjualan_class) editreturpembelian_table.getSelectionModel().getSelectedItem();
                    int data = rp.getId();
                    statement = con.createStatement();
                    statement.executeUpdate("DELETE FROM detail_retur_pembelian WHERE id="+data);
                    initialize2();
                } catch (SQLException ex) {
                System.out.println(ex.getMessage());
                }finally{
    ...
    void editreturpembelian_btnClick(ActionEvent event) {
        ...
        try{
            ...
            if (potongnota_cb.isSelected()){
                statement.executeUpdate("UPDATE nota_pembelian SET retur=retur-"+totalmasuk+"+"+total_txt.getText()+" WHERE id="+notapembelian_id.getText());
                statement.executeUpdate("UPDATE retur_pembelian SET tanggal='"+tanggal.getValue().toString()+"', total_harga="+total_txt.getText()+", potong_nota=TRUE WHERE id="+returpembelian_id.getText());
            }
            else if (potongtunai_cb.isSelected()){
                statement.executeUpdate("UPDATE retur_pembelian SET tanggal='"+tanggal.getValue().toString()+"', total_harga="+total_txt.getText()+", potong_nota=FALSE WHERE id="+returpembelian_id.getText());
                if (totalmasuk != 0){
                    statement.executeUpdate("UPDATE nota_pembelian SET retur=retur-"+totalmasuk+" WHERE id="+notapembelian_id.getText());
                }
            }
            Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
                        ...
                        if (result.get() == buttonTypeOK){
                            try{
                                    con = ConnectionFactory.getConnection();
                                    statement = con.createStatement();
                                    Stage stage = (Stage) editreturpembelian_cancel.getScene().getWindow();
                                    stage.close();
    ...
    @FXML
    void editreturpembelian_cancel_btn(ActionEvent event) {
        Connection con = null;
        Statement statement = null;
        try{
                con = ConnectionFactory.getConnection();
                statement = con.createStatement();
                statement.executeUpdate("DELETE FROM detail_retur_pembelian WHERE id_returpembelian="+returpembelian_id.getText()+ " AND id >= "+masuk);
                Stage stage = (Stage) editreturpembelian_cancel.getScene().getWindow();
                stage.close();
                FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/projectjava/view/pembelian.fxml"));
                Parent root1 = (Parent) fxmlLoader.load();
